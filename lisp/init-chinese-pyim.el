...
(defun pyim-fuzzy-pinyin-adjust-shanghai ()
  ...
  - 'in' and 'ing'"
  (interactive)
  (let ((okey pyim-current-key))
    (cond
     ((string-match-p "[a-z][ei]ng?-.*[a-z][ei]ng?" okey)
      ;; for two fuzzy pinyin characters, just use its SHENMU as key
      (setq pyim-current-key (replace-regexp-in-string "\\([a-z]\\)[ie]ng?" "\\1" okey)))
     (t
      ;; single fuzzy pinyin character
      (cond
       ((string-match-p "[ei]ng" okey)
        (setq pyim-current-key (replace-regexp-in-string "\\([ei]\\)ng" "\\1n" okey)))
       ((string-match-p "[ie]n[^g]*" pyim-current-key)
        (setq pyim-current-key (replace-regexp-in-string "\\([ie]\\)n" "\\1ng" okey))))))
    (message "pyim-current-key=%s okey=%s" pyim-current-key okey)
    (pyim-handle-string (pyim-split-string okey))))
